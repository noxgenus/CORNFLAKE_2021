...
// OR BREAKOUT BOARD USAGE.  SEE RELEVANT COMMENTS IN Andersmmg_TFTLCD.h
...
Andersmmg_TFTLCD::Andersmmg_TFTLCD(
...
Andersmmg_TFTLCD::Andersmmg_TFTLCD(void) : Adafruit_GFX(TFTWIDTH, TFTHEIGHT) {
  init();
}

// Initialization common to both shield & breakout configs
void Andersmmg_TFTLCD::init(void) {
...
void Andersmmg_TFTLCD::begin(uint16_t id) {
  uint8_t i = 0;
...
void Andersmmg_TFTLCD::reset(void) {
...
//  CD_DATA;
  ...
  CS_IDLE;
  // Comment this out when things are working
...
void Andersmmg_TFTLCD::setAddrWindow(int x1, int y1, int x2, int y2) {
  CS_ACTIVE;
  if(driver == ID_932X) {
...
void Andersmmg_TFTLCD::setLR(void) {
  CS_ACTIVE;
...
void Andersmmg_TFTLCD::flood(uint16_t color, uint32_t len) {
  uint16_t blocks;
...
void Andersmmg_TFTLCD::drawFastHLine(int16_t x, int16_t y, int16_t length,
  uint16_t color)
{
  int16_t x2;
...
void Andersmmg_TFTLCD::drawFastVLine(int16_t x, int16_t y, int16_t length,
  uint16_t color)
{
  int16_t y2;
...
void Andersmmg_TFTLCD::fillRect(int16_t x1, int16_t y1, int16_t w, int16_t h, 
  uint16_t fillcolor) {
  int16_t  x2, y2;
...
void Andersmmg_TFTLCD::fillScreen(uint16_t color) {
...
void Andersmmg_TFTLCD::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Andersmmg_TFTLCD::pushColors(uint16_t *data, uint8_t len, boolean first) {
  uint16_t color;
  uint8_t  hi, lo;
  CS_ACTIVE;
...
void Andersmmg_TFTLCD::setRotation(uint8_t x) {
...
uint16_t Andersmmg_TFTLCD::readPixel(int16_t x, int16_t y) {
...
uint16_t Andersmmg_TFTLCD::readID(void) {
...
uint32_t Andersmmg_TFTLCD::readReg(uint8_t r) {
  uint32_t id;
  uint8_t x;
...
uint16_t Andersmmg_TFTLCD::color565(uint8_t r, uint8_t g, uint8_t b) {
...
#ifndef write8
void Andersmmg_TFTLCD::write8(uint8_t value) {
  write8inline(value);
}
#endif

#ifdef read8isFunctionalized
uint8_t Andersmmg_TFTLCD::read8fn(void) {
  uint8_t result;
  read8inline(result);
  return result;
}
...
#ifndef setWriteDir
void Andersmmg_TFTLCD::setWriteDir(void) {
  setWriteDirInline();
}
#endif

#ifndef setReadDir
void Andersmmg_TFTLCD::setReadDir(void) {
  setReadDirInline();
}
#endif

#ifndef writeRegister8
void Andersmmg_TFTLCD::writeRegister8(uint8_t a, uint8_t d) {
  writeRegister8inline(a, d);
}
#endif

#ifndef writeRegister16
void Andersmmg_TFTLCD::writeRegister16(uint16_t a, uint16_t d) {
  writeRegister16inline(a, d);
}
#endif

#ifndef writeRegisterPair
void Andersmmg_TFTLCD::writeRegisterPair(uint8_t aH, uint8_t aL, uint16_t d) {
  writeRegisterPairInline(aH, aL, d);
}
#endif


void Andersmmg_TFTLCD::writeRegister24(uint8_t r, uint32_t d) {
  CS_ACTIVE;
  CD_COMMAND;
  write8(r);
  CD_DATA;
...
void Andersmmg_TFTLCD::writeRegister32(uint8_t r, uint32_t d) {
  CS_ACTIVE;
  CD_COMMAND;
  write8(r);
  CD_DATA;
